...
// Container defined the actual container for the
...
func IsSupported(gitpodCtx *common.RenderContext, serviceAccountName string, clientset *k8s.Clientset) (*bool, error) {
	supported := false
	k8sCtx := context.TODO()

	// Generate the ShiftFS job
	job, err := Job(gitpodCtx, serviceAccountName)
	if err != nil {
		return nil, err
	}
	...
	// Thirty seconds is enough time for this to be deployed and run
	timeoutCtx, cancel := context.WithTimeout(k8sCtx, 30*time.Second)

	// Now we wait for the job to either complete successfully
...
func Job(ctx *common.RenderContext, serviceAccountName string) (*batchv1.Job, error) {
