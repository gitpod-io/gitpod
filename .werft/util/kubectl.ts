...
async function wipePreviewEnvironmentInstaller(namespace: string, shellOpts: ExecOptions) {
    ...
    const hasGitpodConfigmap =
        exec(`kubectl -n ${namespace} get configmap gitpod-app`, { slice, dontCheckRc: true }).code === 0;
...
function deleteAllWorkspaces(namespace: string, shellOpts: ExecOptions) {
    ...
    objs.forEach((o) => {
        try {
            ...
            exec(`kubectl patch pod --namespace ${namespace} ${o} -p '{"metadata":{"finalizers":null}}'`, {
                ...shellOpts,
            });
            const result = exec(
            ...
            if (result.code !== 0) {
                ...
                exec(`kubectl delete pod --namespace ${namespace} ${o} --ignore-not-found=true --force`, {
                    ...shellOpts,
                });
            }
        } catch (err) {
    ...
    for (const resType of ['clusterrole', 'clusterrolebinding', 'podsecuritypolicy']) {
        ...
        for (const obj of objs) {
            promisedDeletes.push(
                exec(`kubectl delete ${resType} ${obj}`, { ...shellOpts, slice, async: true }) as Promise<any>,
            );
        }
    }
...
export function listAllPreviewNamespaces(shellOpts: ExecOptions): string[] {
    return exec(`kubectl get namespaces -l ${IS_PREVIEW_APP_LABEL}=true -o=custom-columns=:metadata.name`, {
        ...shellOpts,
        silent: true,
        async: false,
    })
...
export async function deleteNonNamespaceObjects(namespace: string, destname: string, shellOpts: ExecOptions) {
    exec(`/usr/local/bin/helm3 delete gitpod-${destname} || echo gitpod-${destname} was not installed yet`, {
        ...shellOpts,
    });
