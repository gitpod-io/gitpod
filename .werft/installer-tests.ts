...
interface InfraConfig {
    phase: string; makeTarget: string, description: string
}
...
interface TestConfig {
    DESCRIPTION: string; PHASES: string[];
}
...
const TEST_CONFIGURATIONS: {[name: string]: TestConfig} = {
    ...
    "STANDARD_K3S_TEST": {
        DESCRIPTION: "Deploy Gitpod on a K3s cluster, created on a GCP instance,"
...
installerTests(TEST_CONFIGURATIONS[testConfig])
    .catch((err) => {
        cleanup()
...
export async function installerTests(config: TestConfig) {
    ...
    for(let phase of config.PHASES) {
        ...
        const ret = callMakeTargets(phaseSteps.phase,
                                    phaseSteps.description,
                                    phaseSteps.makeTarget)
        if(ret) {
