...
EOF`);
    ...
    private configureWorkspaces(slice: string) {
        ...
        exec(`yq w -i ${this.options.installerConfigPath} experimental.workspace.classes["default"].prebuildPVC.size 30Gi`, { slice: slice });
        exec(`yq w -i ${this.options.installerConfigPath} experimental.workspace.classes["default"].prebuildPVC.storageClass rook-ceph-block`, { slice: slice });
        exec(`yq w -i ${this.options.installerConfigPath} experimental.workspace.classes["default"].prebuildPVC.snapshotClass csi-rbdplugin-snapclass`, { slice: slice });
        ...
        exec(`yq w -i ${this.options.installerConfigPath} experimental.workspace.classes["small"].prebuildPVC.size 20Gi`, { slice: slice });
        exec(`yq w -i ${this.options.installerConfigPath} experimental.workspace.classes["small"].prebuildPVC.storageClass rook-ceph-block`, { slice: slice });
        exec(`yq w -i ${this.options.installerConfigPath} experimental.workspace.classes["small"].prebuildPVC.snapshotClass csi-rbdplugin-snapclass`, { slice: slice });        
