...
export class MessageBusIntegration extends AbstractMessageBusIntegration {
  ...
  async connect(): Promise<void> {
    ...
    await this.channel.bindExchange(
      MessageBusHelperImpl.WORKSPACE_EXCHANGE,
      MessageBusHelperImpl.WORKSPACE_EXCHANGE_LOCAL,
      '#',
    );
  }

  listenForWorkspaceStatusUpdates(
    topic: string,
    callback: (ctx: TraceContext, status: WorkspaceStatus.AsObject) => void,
  ): Disposable {
...
class WorkspaceStatusUpdateListener extends AbstractTopicListener<WorkspaceStatus.AsObject> {
  constructor(
    listener: TopicListener<WorkspaceStatus.AsObject>,
