...
import {
  ...
  TLSConfig,
  AdmissionConstraint,
  AdmissionConstraintHasRole,
  AdmissionPreference,
  AdmissionPreferenceUserLevel,
...
export class ClusterService implements IClusterServiceServer {
  ...
  public register(
    ...
    this.queue.enqueue(async () => {
      try {
        ...
        const admissionPreferences = call.request
          .getAdmissionPreferenceList()
          .map(mapAdmissionPreference)
          .filter((c) => !!c) as AdmissionPreference[];
  ...
  protected triggerReconcile(action: string, name: string) {
    ...
    this.bridgeController.runReconcileNow().catch((err) => log.error('error during forced reconcile', err, payload));
  }
}
