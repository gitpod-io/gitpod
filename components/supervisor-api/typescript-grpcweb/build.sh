...
mkdir -p src
protoc $PROTO_INCLUDE --plugin="protoc-gen-ts=`which protoc-gen-ts`" --js_out="import_style=commonjs,binary:src" --ts_out="service=grpc-web:src" -I${PROTOLOC:-..} ${PROTOLOC:-..}/*.proto