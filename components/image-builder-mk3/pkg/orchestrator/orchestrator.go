		...
		swr, err = o.wsman.StartWorkspace(ctx, &wsmanapi.StartWorkspaceRequest{
			...
			Spec: &wsmanapi.StartWorkspaceSpec{
				...
				Envvars: []*wsmanapi.EnvironmentVariable{
					...
					{Name: "BOB_EXTERNAL_BUILDKITD", Value: "tcp://localhost:9999"},
					{Name: "WORKSPACEKIT_RING2_ENCLAVE", Value: "/app/bob build"},
					{Name: "GITPOD_TASKS", Value: `[{"name": "build", "init": "sudo -E /app/bob daemon tcp://0.0.0.0:9999"}]`},
