...
type Workspace struct {
	...
	// UpperdirLocation is the absolute ws-sync container-relative path to the workspace container's upperdir.
...
// MarkInitDone marks this workspace as initialized and writes this workspace to disk so that it can be restored should ws-sync crash/be restarted
func (s *Workspace) MarkInitDone(ctx context.Context) (err error) {
	...
	// something different than a restarted ws-sync.
	err = s.persist()
	if err != nil {
