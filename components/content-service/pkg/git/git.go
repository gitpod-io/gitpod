...
func (c *Client) GitWithOutput(ctx context.Context, subcommand string, args ...string) (out []byte, err error) {
	...
	cmd0 := exec.Command("git", "version")
	cmd0.Dir = c.Location
	cmd0.Env = env
	res0, err := cmd0.CombinedOutput()
	log.Infof("git version %v: %s", fullArgs, res0)
	cmd1 := exec.Command("ls", "-lrtha", "./*")
	cmd1.Dir = c.Location
	cmd1.Env = env
	res1, err := cmd1.CombinedOutput()
	log.Infof("ls -lrtha ./* before git command %v: %s", fullArgs, res1)
	cmd3 := exec.Command("ls", "-lrtha")
	cmd3.Dir = c.Location
	cmd3.Env = env
	res3, err := cmd3.CombinedOutput()
	log.Infof("ls -lrtha * before git command %v: %s", fullArgs, res3)
