...
class TestClientProvider {
      ...
      .toDynamicValue((): WorkspaceManagerClientProviderSource => {
        const cluster: WorkspaceCluster[] = [
          ...
          },
          {
            ...
            state: 'available',
            url: '',
            admissionConstraints: [{ type: 'has-feature-preview' }],
          },
          {
            name: 'con2',
            govern: true,
  ...
  public async testGetStarterWorkspaceCluster() {
    this.expectInstallations(
      ['a1', 'a2', 'a3'],
      await this.provider.getAvailableStartCluster({} as User, {} as Workspace, {} as WorkspaceInstance),
    );
    this.expectInstallations(
      ['a1', 'a2', 'a3', 'con1'],
      await this.provider.getAvailableStartCluster(
        {
  ...
  public async getStartManager() {
    const { installation } = await this.provider.getStartManager({} as User, {} as Workspace, {} as WorkspaceInstance, [
      'a2',
      'a3',
    ]);
