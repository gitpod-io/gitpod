...
class TestClientProvider {
    ...
    public testConstraintHasMoreResources() {
        ...
        expect(
            constraintMoreResources(clusters, {} as ExtendedUser, {} as Workspace, {} as WorkspaceInstance).map(
                (c) => c.name,
            ),
            'more resources',
        ).to.be.eql(['a1']);
        expect(
            constraintInverseMoreResources(clusters, {} as ExtendedUser, {} as Workspace, {} as WorkspaceInstance).map(
                (c) => c.name,
            ),
            'inverse more resources',
        ).to.be.eql(['b1']);
    }
