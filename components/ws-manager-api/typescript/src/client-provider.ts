...
export class WorkspaceManagerClientProvider implements Disposable {
  ...
  public async getStartManager(
    ...
    exceptInstallations?: string[],
  ): Promise<{ manager: PromisifiedWorkspaceManagerClient; installation: string }> {
  ...
  public async getAvailableStartCluster(
    user: User,
    workspace: Workspace,
    instance: WorkspaceInstance,
  ): Promise<WorkspaceClusterWoTLS[]> {
    const allClusters = await this.source.getAllWorkspaceClusters();
    const availableClusters = allClusters
      .filter((c) => c.score >= 0 && c.state === 'available')
      .filter(admissionConstraintsFilter(user, workspace, instance));
...
function admissionConstraintsFilter(
  user: User,
  workspace: Workspace,
  instance: WorkspaceInstance,
): (c: WorkspaceClusterWoTLS) => boolean {
