    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        if (!(await tableExists(queryRunner, "d_b_code_sync_collection"))) {
            await queryRunner.query(
                "CREATE TABLE IF NOT EXISTS `d_b_code_sync_collection` (`userId` char(36) NOT NULL, `collection` char(36) NOT NULL, `_lastModified` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY(`userId`, `collection`), KEY `ind_dbsync` (`_lastModified`)) ENGINE=InnoDB",
            );
        }
