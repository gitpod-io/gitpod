...
const column = "organisationId";
const orgIndex = "idx_organisationId";
const idOrgCompositeIndex = "idx_id_organisationId";
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        if (!(await columnExists(queryRunner, table, column))) {
            await queryRunner.query(
                `ALTER TABLE ${table} ADD COLUMN ${column} varchar(255) NOT NULL, ALGORITHM=INPLACE, LOCK=NONE`,
            );
        }
