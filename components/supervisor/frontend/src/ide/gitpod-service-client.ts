...
export async function create(): Promise<GitpodServiceClient> {
    ...
    //#region info
    const listener = await (window.gitpod as any).service.listenToInstance(wsUrl.workspaceId);
    //#endregion
    ...
    //#region user
    const user = await (window.gitpod as any).service.server.getLoggedInUser();
    //#endregion
