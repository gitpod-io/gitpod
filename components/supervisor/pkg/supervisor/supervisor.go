...
func createExposedPortsImpl(cfg *Config, gitpodService serverapi.APIInterface) ports.ExposedPortsInterface {
	...
	return ports.NewGitpodExposedPorts(cfg.WorkspaceID, cfg.WorkspaceInstanceID, cfg.WorkspaceUrl, cfg.DebugWorkspace, gitpodService)
}
...
func runIDEReadinessProbe(cfg *Config, ideConfig *IDEConfig, ide IDEKind) (desktopIDEStatus *DesktopIDEStatus) {
	...
	case ReadinessHTTPProbe:
		...
		if cfg.DebugWorkspace && ide == WebIDE {
			log.WithField("debugWorkspace", "true").
				WithField("workspaceUrl", cfg.WorkspaceUrl).
				Infof("The debug workspace is available on: %s", cfg.WorkspaceUrl)
		}
		if ide != DesktopIDE {
			return
		}
