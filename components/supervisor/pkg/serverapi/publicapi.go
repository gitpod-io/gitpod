...
type APIInterface interface {
	...
	UpdateRepoStatus(ctx context.Context, status *gitpod.WorkspaceInstanceRepoStatus) (err error)
...
func (s *Service) UpdateRepoStatus(ctx context.Context, status *gitpod.WorkspaceInstanceRepoStatus) (err error) {
	startTime := time.Now()
	usePublicApi := s.usePublicAPI(ctx)
	defer func() {
		s.apiMetrics.ProcessMetrics(usePublicApi, "UpdateRepoStatus", err, startTime)
	}()
	if s == nil {
	...
	if !usePublicApi {
		return s.gitpodService.UpdateRepoStatus(ctx, workspaceID, status)
	}
	service := v1.NewWorkspacesServiceClient(s.publicAPIConn)
	payload := &v1.UpdateRepoStatusRequest{
		WorkspaceId: workspaceID,
	}
	if status != nil {
		payload.Status = &v1.RepoStatus{
	...
	_, err = service.UpdateRepoStatus(ctx, payload)
	return
}
