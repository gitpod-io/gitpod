...
export class GitpodHostUrlTest {
  ...
  @test public async testWithoutWorkspacePrefix2() {
    expect(GitpodHostUrl.fromWorkspaceUrl('https://gitpod-staging.com/').withoutWorkspacePrefix().toString()).to.equal(
  ...
  @test public async testWithoutWorkspacePrefix6() {
    expect(
      GitpodHostUrl.fromWorkspaceUrl('https://gray-rook-5523v5d8.ws-dev.abc-nice-brunch-4224.staging.gitpod-dev.com/')
        .withoutWorkspacePrefix()
        .toString(),
    ).to.equal('https://abc-nice-brunch-4224.staging.gitpod-dev.com/');
  }
