    ...
    (async () => {
      ...
      if (project) {
        ...
        if (configString) {
          ...
          setEditorError(<span>A Gitpod configuration already exists in the project's <a href={project.cloneUrl}>repository</a>. Please <a href={`/#${project.cloneUrl}`}>edit it in Gitpod</a> instead.</span>);
  ...
  return <div className="flex flex-col mt-24 mx-auto items-center">
    ...
    <div className="mt-4 w-full flex">
      <div className="flex-1 m-8">
        {editorError && <AlertBox>{editorError}</AlertBox>}
        ...
        <Suspense fallback={<div />}>
          {selectedEditor === '.gitpod.yml' &&
            <MonacoEditor classes="mt-4 w-full h-72" disabled={isEditorDisabled} language="yaml" value={gitpodYml} onChange={setGitpodYml} />}
          {selectedEditor === '.gitpod.Dockerfile' &&
            <MonacoEditor classes="mt-4 w-full h-72" disabled={isEditorDisabled} language="dockerfile" value={dockerfile} onChange={setDockerfile} />}
        </Suspense>
