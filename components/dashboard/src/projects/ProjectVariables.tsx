      ...
      {envVars.length === 0 ? (
        <div className="bg-gray-100 dark:bg-gray-800 rounded-xl w-full py-28 flex flex-col items-center">
          ...
          <p className="text-center pb-6 text-gray-500 text-base w-96">
            All <strong>project-specific environment variables</strong> will be visible in prebuilds and optionally in
    ...
    >
      ...
      <div className="border-t border-b border-gray-200 dark:border-gray-800 -mx-6 px-6 py-4 flex flex-col">
        <AlertBox>
          ...
          <br />
          Even if <strong>Hide Variable in Workspaces</strong> is enabled, anyone with read access to your repository
          can access secret values if they are printed in the terminal, logged, or persisted to the file system.
        </AlertBox>
        {error && (
          <AlertBox className="mt-4">{String(error).replace(/Error: Request \w+ failed with message: /, '')}</AlertBox>
        )}
        <div className="mt-8">
          <h4>Name</h4>
          <input
