        ...
        {showAuthBanner ? (
          <div className="mt-8 rounded-xl text-gray-500 bg-gray-50 dark:bg-gray-800 flex-col">
            <div className="p-8 text-center">
              ...
              <button className={`primary mr-2 py-2`} onClick={() => onConfirmShowAuthModal(showAuthBanner.host)}>
                Authorize Provider
              </button>
            </div>
          </div>
                    ...
                    <Item key={`branch-${index}-${branch.name}`} className="grid grid-cols-3 group">
                      ...
                      <ItemField className="flex items-center my-auto">
                        <a
                          ...
                          href={
                            prebuild
                              ? `/${!!team ? 't/' + team.slug : 'projects'}/${projectSlug}/${prebuild.info.id}`
                              : 'javascript:void(0)'
                          }
                        >
                          {prebuild ? (
                        ...
                        <a href={gitpodHostUrl.withContext(`${branch.url}`).toString()}>
                          <button className={`primary mr-2 py-2 opacity-0 group-hover:opacity-100`}>
                            New Workspace
                          </button>
                        </a>
