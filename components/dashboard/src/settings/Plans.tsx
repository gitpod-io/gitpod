...
export default function () {
  ...
  if (currentPlan.chargebeeId === freePlan.chargebeeId) {
    planCards.push(
      <PlanCard isDisabled={!!assignedTs || pendingChargebeeCallback} plan={freePlan} isCurrent={!!accountStatement}>
        {openSourceFeatures}
      </PlanCard>,
    );
  } else {
    ...
    if (scheduledDowngradePlanId === targetPlan.chargebeeId && !pendingDowngradeCancellation) {
      bottomLabel = (
        <p className="text-green-600">
          Downgrade scheduled
          <br />
          <a className="text-blue-light leading-6" href="javascript:void(0)" onClick={() => confirmCancelDowngrade()}>
            Cancel
          </a>
        </p>
      );
  ...
  if (currentPlan.chargebeeId === personalPlan.chargebeeId) {
    const bottomLabel =
      'pendingSince' in currentPlan ? <p className="text-green-600 animate-pulse">Upgrade in progress</p> : undefined;
    planCards.push(
      <PlanCard
        isDisabled={!!assignedTs || pendingChargebeeCallback}
    ...
    <div>
      <PageWithSubMenu subMenu={settingsMenu} title="Plans" subtitle="Manage account usage and billing.">
        {showPaymentUI && (
          <div className="w-full text-center">
            <p className="text-xl text-gray-500">
              ...
              <span className="font-bold">{Plans.getById(assignedTs?.planId)?.name || currentPlan.name}</span> plan.
            </p>
            {!assignedTs && (
        ...
        <InfoBox className="w-2/3 mt-14 mx-auto">
          If you are interested in purchasing a plan for a team, purchase a Team plan with one centralized billing.{' '}
              ...
              {!Plans.isFreePlan(currentPlan.chargebeeId) && (
                <InfoBox className="mb-4">
                  ...
                  {(confirmUpgradeToPlan.currency === 'EUR' ? '€' : '$') +
                    (confirmUpgradeToPlan.pricePerMonth - applyCoupons(currentPlan, appliedCoupons).pricePerMonth)}
                  ). The new total will be effective from the next billing cycle.
                </InfoBox>
              )}
              <AlertBox className="mb-4">
                Total: {(confirmUpgradeToPlan.currency === 'EUR' ? '€' : '$') + confirmUpgradeToPlan.pricePerMonth} per
                month
              </AlertBox>
            </div>
            ...
            <div className="border-t border-b border-gray-200 dark:border-gray-800 mt-4 -mx-6 px-6 py-2">
              ...
              <InfoBox className="mb-4">
                {!Plans.isFreePlan(confirmDowngradeToPlan.chargebeeId) ? (
                  ...
                  <span>
                    Your account will downgrade to {confirmDowngradeToPlan.name}. The remaining hours in your current
                    plan will be available to use until the next billing cycle.
                  </span>
                )}
              </InfoBox>
            </div>
...
function guessCurrentBillingCycle(currentPlan: Plan, accountStatement?: AccountStatement): Date[] {
  ...
  try {
    ...
    const credit = accountStatement.credits.find(
      (c) =>
        c.date < now && c.expiryDate >= now && (c.description as CreditDescription)?.planId === currentPlan.chargebeeId,
    );
    if (!!credit) {
