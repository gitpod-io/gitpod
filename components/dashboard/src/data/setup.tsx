...
function createIDBPersister(idbValidKey: IDBValidKey = "gitpodQueryClient"): Persister {
    ...
    /**
     * Shallow check if the data is a protobuf message.
     * We should not persist protobuf messages as they are not JSON serializable.
     * Instead we should persist the plain message.
     */
    function throwIfMessageIsProtobuf(data: any, type: string) {
        if (data instanceof Array) {
            data = data[0];
        }
        if (data instanceof Message) {
            throw new Error(
                `cannot persist ${data.getType().typeName} protobuf message, use toPlainMessage in ${type}}`,
            );
        }
    }
    ...
    const debouncedSet = debounce(
        async (client: PersistedClient) => {
            client.clientState.mutations.forEach((m) => throwIfMessageIsProtobuf(m.state.data, "useMutation"));
            client.clientState.queries.forEach((q) => throwIfMessageIsProtobuf(q.state.data, "useQuery"));
    ...
    return {
        persistClient: async (client: PersistedClient) => {
            ...
            try {
                ...
                console.error("unable to persist query client:", e);
        ...
        restoreClient: async () => {
            try {
                ...
                console.error("unable to load query client from cache:", e);
                persistanceActive = false;
            }
        },
        removeClient: async () => {
            try {
                ...
                console.error("unable to remove query client:", e);
                persistanceActive = false;
            }
        },
    };
