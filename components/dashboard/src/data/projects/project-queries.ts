...
export const useUpdateProject = () => {
    ...
    return useMutation<void, Error, PartialProject>(async (changes) => {
        ...
        await getGitpodService().server.updateProjectPartial(changes);

        // Invalidate project
        client.invalidateQueries(getProjectQueryKey(org.id, changes.id));
...
export const useDeleteProject = () => {
    ...
    return useMutation<void, Error, string>(async (id: string) => {
        if (!org) {
            throw new Error("No org currently selected");
        }

        await getGitpodService().server.deleteProject(id);
