...
export namespace MenuEntry {
  export const ACCESS_CONTROL: MenuEntry = {
    ...
    open: (loc) =>
      new GitpodHostUrl(loc.toString()).asAccessControl().toString(),
  };
  ...
  export const LOG_OUT: MenuEntry = {
    caption: "Log Out",
    open: (loc) =>
      new GitpodHostUrl(loc.toString())
        .withApi({ pathname: "/logout" })
        .toString(),
  };
...
export class MainMenu extends React.Component<MainMenuProps, MainMenuState> {
  ...
  render() {
    ...
    if (this.props.userLoaded) {
      if (this.props.user) {
        const menuItems = this.props.menuEntries.map((e) => (
          <MenuItem key={e.caption} onClick={(event) => this.handleClose(e)}>
            {e.caption}
          </MenuItem>
        ));
        ...
        const abbr = (name: string) =>
          name
            ...
            .join("");
        const avatarText = this.props.user.avatarUrl
          ? null
          : abbr(this.props.user.name || "?");
        ...
        const items = this.props.authProviders
          .filter((a) => !a.disallowLogin)
          .map((a) => a.host);
        if (items.length === 1) {
          const host = items[0];
          userSection = (
      ...
      userSection = <Fade in={true}>{userSection || <div></div>}</Fade>;
    }
        ...
        <Toolbar className="content toolbar">
          <div className="gitpod-logo">
            <a
              href={
                this.props.branding
                  ? this.props.branding.homepage
                  : "javascript:void(0)"
              }
            >
              <img
                src={getLogoPath(this.props.branding)}
                aria-hidden="true"
                className="logo"
              />
            </a>
          ...
          <div className="gitpod-links">
            <div className="link-container">
              ...
              {!this.props.linksOverride &&
                (this.props.branding
                  ? this.props.branding.links.header
                  : []
                ).map(({ name, url }: Branding.Link) =>
                  this.createAnchor(name, url)
                )}
            </div>
          </div>
          <div style={{ display: "flex" }}>
            <MobileMenu
              items={
                this.props.branding ? this.props.branding.links.header : []
              }
            />
  ...
  protected createAnchor(name: string, url: string) {
    if (url.startsWith("/")) {
      return (
        <a href={url} key={"a-" + name}>
          {name}
        </a>
      );
    } else {
      return (
        <a href={url} key={"a-" + name} target="_blank" rel="noopener">
          {name}
        </a>
      );
    }
