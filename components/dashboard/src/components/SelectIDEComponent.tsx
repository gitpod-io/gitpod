    ...
    const getElements = useMemo(() => {
        return (search: string) => {
            ...
            for (const ide of options.filter(
                (ide) =>
                    `${ide.label}${ide.title}${ide.notes}${ide.id}`.toLowerCase().indexOf(search.toLowerCase()) !== -1,
            )) {
                result.push({
                    id: ide.id,
            ...
            <div className="flex-col ml-1 mt-1 flex-grow">
                ...
                <div className="flex text-xs text-gray-500 dark:text-gray-400">
                    ...
                    {label && (
                        <>
                            <div className="mx-1">&middot;</div>
                            <div>{label && label[0].toLocaleUpperCase() + label.slice(1)}</div>
                        </>
                    )}
...
function IdeOptionElementInDropDown(p: IdeOptionElementProps): JSX.Element {
    ...
    const version = useLatest ? option.latestImageVersion : option.imageVersion;
    const label = option.type && option.type[0].toLocaleUpperCase() + option.type.slice(1);
