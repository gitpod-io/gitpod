...
export function StartWorkspaceModal(p: StartWorkspaceModalProps) {
  ...
  useEffect(() => {
    !p.visible && setSelection(computeSelection());
  }, [p.visible, p.recent, p.selected]);
  ...
  return (
    <Modal onClose={p.onClose} visible={p.visible}>
        ...
        <div className="space-y-2 mt-4 overflow-y-scroll h-80 pr-2">
          {list.length > 0 ? (
            ...
            <div className="flex flex-col pt-10 items-center px-2">
              ...
              <button onClick={() => setSelection('Examples')} className="font-medium mt-8">
                Select Example
              </button>
            </div>
          ) : (
