...
export class GithubSubscriptionReconciler {
  ...
  protected async reconcilePlan(plan: Plan, token: string) {
    ...
    for (const sub of subscriptionsInDB) {
      ...
      if (!listing) {
        ...
        log.warn({ userId: sub.userId }, 'Found subscription which GitHub does not know off. Ending subscription.', {
          subscription: sub,
        });
  ...
  public async reconcileEvent(
    ...
    if (!success) {
      log.debug({ userId }, 'subscription mapper did not succeed for GitHub market purchase event. See errors above.', {
        evt,
      });
      return;
    }
