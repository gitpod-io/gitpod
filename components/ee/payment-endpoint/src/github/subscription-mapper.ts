...
export type MarketplaceEventPendingChangeCancelled =
    Webhooks.EmitterWebhookEvent<'marketplace_purchase.pending_change_cancelled'>;
...
export class GithubSubscriptionMapper {
    ...
    public mapSubscriptionPurchase(
        user: User,
        accountID: number,
        startDate: string,
        plan: Plan,
    ...
    public mapSubscriptionChange(user: User, context: ChangeContext, model: SubscriptionModel) {
        ...
        if (!oldSubscription) {
            log.error(
                { userId: user.id },
                "asked to make a subscription change but can't find previous subscription. We just missed a subscription change.",
                context,
            );
            ...
            log.debug({ userId: user.id }, 'downgraded subscription', {
                subscription: oldSubscription.uid,
                plan: newPlan,
            });
        } else if (oldSubscription.amount < newAmount) {
            // Upgrade
            ...
            log.debug({ userId: user.id }, 'upgraded subscription', {
                subscription: oldSubscription.uid,
                plan: newPlan,
            });
        } else {
            ...
            oldSubscription.planId = newPlan.chargebeeId;
            model.update(oldSubscription);
            log.debug({ userId: user.id }, 'updated subscription', {
                subscription: oldSubscription.uid,
                plan: newPlan,
            });
        }
    ...
    protected mapChangedEvent(
        user: User,
        evt: Webhooks.EmitterWebhookEvent<'marketplace_purchase.changed'>,
        model: SubscriptionModel,
    ) {
    ...
    protected mapPendingChangeCancelledEvent(
        user: User,
        evt: MarketplaceEventPendingChangeCancelled,
        model: SubscriptionModel,
    ) {
        const context = this.getChangeContext(evt.payload, user.id, model);
        if (!context) {
        ...
        if (!oldSubscription) {
            log.warn(
                { userId: user.id },
                "received change canceled event but can't find previous subscription - adding to pending events",
                evt,
            );
    ...
    protected getChangeContext(
        payload:
            | MarketplaceEventPendingChange['payload']
            | MarketplaceEventChanged['payload']
            | MarketplaceEventPendingChangeCancelled['payload'],
        userId: string,
        model: SubscriptionModel,
