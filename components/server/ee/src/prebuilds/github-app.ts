...
export class GithubApp {
  ...
  protected async buildApp(app: Probot, options: ApplicationFunctionOptions) {
    ...
    options.getRouter &&
      options.getRouter('/pbs').get('/*', (req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  protected async handlePushEvent(ctx: Context<'push'>): Promise<void> {
    ...
    try {
      ...
      const owner =
        installationId &&
        ((await this.findProjectOwner(cloneURL)) || (await this.findInstallationOwner(installationId)));
      if (!owner) {
      ...
      this.prebuildManager
        .startPrebuild({ span }, { user, contextURL, cloneURL: repo.clone_url, commit: pl.after, branch, project })
  ...
  protected async onPrAddCheck(
    ...
    ctx: Context<'pull_request.opened' | 'pull_request.synchronize' | 'pull_request.reopened'>,
    start: Promise<StartPrebuildResult> | undefined,
  ) {
    if (!start) {
      return;
    }
  ...
  protected onPrStartPrebuild(
    tracecContext: TraceContext,
    config: WorkspaceConfig | undefined,
    owner: { user: User; project?: Project },
    ctx: Context<'pull_request.opened' | 'pull_request.synchronize' | 'pull_request.reopened'>,
    ...
    if (runPrebuild) {
      ...
      log.debug({ userId: owner.user.id }, `Not running prebuild, the user did not enable it for this context`, {
        contextURL,
        owner,
      });
      return;
  ...
  protected async findInstallationOwner(
    installationId: number,
  ): Promise<{ user: User; project?: Project } | undefined> {
    // Legacy mode
    //
