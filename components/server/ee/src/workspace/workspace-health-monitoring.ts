...
export class WorkspaceHealthMonitoring {
    ...
    async probeAllRunningWorkspaces(ctx: TraceContext): Promise<ProbeResult[]> {
        ...
        try {
            const workspaces = await this.workspaceDb
                .trace({ span })
                .findRunningInstancesWithWorkspaces(this.config.installationShortname);
            const workspacesFilter = (ws: RunningWorkspaceInfo) =>
                !!ws.latestInstance.ideUrl && ws.latestInstance.status.phase === 'running';
            const resultMapper = async (ws: RunningWorkspaceInfo) => {
                ...
                if (!result.ok) {
                    ...
                    if (wsi && wsi.status.phase === 'running') {
                        ...
                        log.info(
                            { instanceId: ws.latestInstance.id, workspaceId: ws.workspace.id },
                            'Workspace status phase changed during health check.',
                        );
    ...
    protected async probeWorkspaceOnDifferentProbeURLs(
        ctx: TraceContext,
    ...
    protected async probeWorkspace(ctx: TraceContext, wsi: WorkspaceInstance, probeURL: URL): Promise<ProbeResult> {
        ...
        try {
            ...
            log.error({ instanceId: wsi.id, workspaceId: wsi.workspaceId }, 'workspace health check failed', err, {
