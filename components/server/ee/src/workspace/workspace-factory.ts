        ...
        try {
            ...
            if (context.commitHistory && context.commitHistory.length > 0) {
                ...
                if (match) {
                    ...
                    //TODO doing side effects on objects back and forth is complicated and error-prone. We should rather make sure we pass in the config when creating the prebuiltWorkspace.
                    ws.config = config;
                }
            }
            if (!ws) {
