    ...
    protected async createForStartPrebuild(
        ...
        try {
            ...
            const existingPWS = await this.db
                .trace({ span })
                .findPrebuiltWorkspaceByCommit(commitContext.repository.cloneUrl, commitContext.revision);
            if (existingPWS) {
            ...
            for (const parent of context.commitHistory || []) {
                const parentPrebuild = await this.db
                    .trace({ span })
                    .findPrebuiltWorkspaceByCommit(commitContext.repository.cloneUrl, parent);
                if (!parentPrebuild) {
                    continue;
                }
