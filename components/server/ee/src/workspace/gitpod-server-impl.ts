...
export class GitpodServerEEImpl extends GitpodServerImpl {
  ...
  protected async listenForPrebuildUpdates() {
    ...
    for (const projectId of projects) {
      this.disposables.push(
        this.localMessageBroker.listenForPrebuildUpdates(projectId, (ctx: TraceContext, update: PrebuildWithStatus) => {
  ...
  protected async guardSnaphotAccess(ctx: TraceContext, userId: string, workspaceId: string): Promise<Workspace> {
    ...
    await this.guardAccess(
      { kind: 'snapshot', subject: undefined, workspaceOwnerID: workspace.ownerId, workspaceID: workspace.id },
      'create',
    );
  ...
  async getSnapshots(ctx: TraceContext, workspaceId: string): Promise<string[]> {
    ...
    await Promise.all(
      snapshots.map((s) =>
        this.guardAccess({ kind: 'snapshot', subject: s, workspaceOwnerID: workspace.ownerId }, 'get'),
      ),
    );
  ...
  protected async findPrebuiltWorkspace(
    ...
    const prebuiltWorkspace = await this.workspaceDb
      .trace(ctx)
      .findPrebuiltWorkspaceByCommit(cloneUrl, context.revision);
    ...
    if (prebuiltWorkspace.state === 'available') {
      ...
      log.info(
        logCtx,
        `Found prebuilding (starting=${result.runningWorkspacePrebuild!.starting}) workspace for ${cloneUrl}:${
          context.revision
        }`,
        logPayload,
      );
  ...
  protected readonly findAssigneeThrottled: ThrottledFunction<any[], FindUserByIdentityStrResult> = pThrottle(
  ...
  public async createProject(ctx: TraceContext, params: CreateProjectParams): Promise<Project> {
    ...
    this.disposables.push(
      this.localMessageBroker.listenForPrebuildUpdates(project.id, (ctx: TraceContext, update: PrebuildWithStatus) => {
        this.client?.onPrebuildUpdate(update);
      }),
    );
    return project;
