...
export class SnapshotService {
  ...
  public async pickupAndDriveFromDbIfWeAreLeader() {
    ...
    for (const snapshot of pendingSnapshots) {
      ...
      if (!workspace) {
        log.error({ workspaceId: snapshot.originalWorkspaceId }, `snapshots: unable to find workspace for snapshot`, {
          snapshotId: snapshot.id,
        });
        continue;
      }

      SafePromise.catchAndLog(this.driveSnapshotCached({ workspaceOwner: workspace.ownerId, snapshot }), {
