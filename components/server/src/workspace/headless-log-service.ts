...
export class HeadlessLogService {
  ...
  public async getHeadlessLogURLs(
    ...
    if (isSupervisorAvailableSoon(wsi)) {
      ...
      const streamIds = await this.retryWhileInstanceIsRunning(
        wsi,
        () => this.supervisorListHeadlessLogs(wsi),
  ...
  async streamWorkspaceLog(
    ...
    const doStream = (cancelRetry: () => void) =>
   ...
   * @returns
   */
  protected async retryWhileInstanceIsRunning<T>(
    wsi: WorkspaceInstance,
