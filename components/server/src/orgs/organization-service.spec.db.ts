...
describe("OrganizationService", async () => {
    ...
    it("should allow owners to get an invite", async () => {
        ...
        const invite2 = await os.getOrCreateGenericInvite(owner.id, org.id);
        expect(invite2.id).to.equal(invite.id);

        const invite3 = await os.resetGenericInvite(owner.id, org.id);
    ...
    it("should not allow strangers to get an invite", async () => {
        ...
        try {
            await os.getOrCreateGenericInvite(stranger.id, org.id);
