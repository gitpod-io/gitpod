            ...
            (async () => {
              try {
                const resolvedPlugin =
                  (await this.resolveFromUploaded(pluginId)) || (await this.resolveFromOpenVSX(parsed, vsxRegistryUrl));
  ...
  > {
    try {
      ...
      if (response.status !== 200) {
        log.error(
          `Failed to find extension '${name}@${version || 'latest'}' with '${queryHref}': ${response.status} (${
            response.statusText
          }).`,
        );
        return undefined;
