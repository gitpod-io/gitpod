...
export class GenericAuthProvider implements AuthProvider {
      ...
      this.readAuthUserSetup = async (accessToken: string, tokenResponse: object) => {
        ...
        try {
          promise = runInNewContext(
            `tokenResponse = ${JSON.stringify(tokenResponse)} || {}; (${configFn})("${accessToken}", tokenResponse)`,
            { fetch, console },
  ...
  async callback(request: express.Request, response: express.Response, next: express.NextFunction): Promise<void> {
    ...
    if (isAlreadyLoggedIn) {
      if (!authFlow) {
        log.warn(cxt, `(${strategyName}) User is already logged in. No auth info provided. Redirecting to dashboard.`, {
          request,
          clientInfo,
        });
    ...
    try {
      result = await new Promise((resolve) => {
        const authenticate = passport.authenticate(this.getStrategy() as any, (...params: Parameters<VerifyCallback>) =>
          resolve(params),
        );
      ...
      if (currentGitpodUser) {
        ...
        if (currentIdentity && currentIdentity.authId !== candidate.authId) {
          log.warn(`User is trying to connect with another provider identity.`, {
            ...
            AuthException.create('authId-mismatch', 'Auth ID does not match with existing provider identity.', {}),
            undefined,
          );
          return;
        }
