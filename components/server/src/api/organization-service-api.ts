...
export class OrganizationServiceAPI implements ServiceImpl<typeof OrganizationServiceInterface> {
    ...
    async updateOrganizationSettings(
        ...
        if (settings.restrictedEditorNames.length > 0 && !settings.updateRestrictedEditorNames) {
        ...
        if (settings.allowedWorkspaceClasses.length > 0 && !settings.updateAllowedWorkspaceClasses) {
            throw new ApplicationError(
                ErrorCodes.BAD_REQUEST,
        ...
        if (settings.roleRestrictions.length > 0 && !settings.updateRoleRestrictions) {
            throw new ApplicationError(
                ErrorCodes.BAD_REQUEST,
        ...
        try {
            update = this.apiConverter.fromOrganizationSettings(settings);
        } catch (err) {
