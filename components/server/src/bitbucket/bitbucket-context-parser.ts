...
export class BitbucketContextParser extends AbstractContextParser implements IContextParser {
  ...
  protected async handlePullRequestContext(
    ...
    try {
      const api = await this.api(user);
      const pr = (
        await api.repositories.getPullRequest({ workspace: owner, repo_slug: repoName, pull_request_id: more.nr })
      ).data;
      more.title = pr.title || '';
  ...
  protected async handleIssueContext(
    ...
    try {
      const api = await this.api(user);
      const issue = (await api.repositories.getIssue({ workspace: owner, repo_slug: repoName, issue_id: `${more.nr}` }))
        .data;
